...
export default function TerminalInterface() {
                ...
                output = (
                    <div className="text-gray-300">
                        ...
                        <div className="grid grid-cols-[100px_1fr] gap-2">
                            ...
                            <span className="text-green-400">cat</span> <span>[file] Read file content</span>
                            <span className="text-green-400">clear</span> <span>Clear terminal screen</span>
                            <span className="text-green-400">whoami</span> <span>Print current user</span>
                            <span className="text-green-400">date</span> <span>Print current date</span>
                            <span className="text-green-400">exit</span> <span>Return to GUI mode</span>
                        </div>
                    </div>
                )
                break
                ...
                output = (
                    <div className="flex gap-4 text-blue-400 font-bold">
                        {Object.keys(FILESYSTEM).map(key => (
                            <span key={key} className={key.includes('.') ? 'text-gray-200' : 'text-blue-400'}>
                                {key}
                            </span>
                        ))}
                    </div>
    ...
    return (
        <div
            className="font-mono text-sm md:text-base min-h-screen bg-black text-green-500 p-4 md:p-8 overflow-hidden"
                    ...
                    <div key={index} className="mb-2">
                        {item.command && (
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                                <span className="text-blue-500 font-bold">┌──(guest㉿asif-portfolio)-[{currentPath}]</span>
                        ...
                        <div className="ml-0 md:ml-4 text-gray-300">{item.output}</div>
                    </div>
                ))}
